...
class LearnVector(nn.Module):
    def __init__(self, in_planes, planes, act_layer=F.relu, concatenate=True, inplace_activation=False):
        ...
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=(1, 1), padding=(0, 0), bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=(1, 1), padding=(0, 0), bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
    ...
    def forward(self, x):
        ...
        out = self.act_layer(self.bn1(self.conv1(x)), inplace=self.inplace)
        out = self.act_layer(self.bn2(self.conv2(out)), inplace=self.inplace)
        if self.concatenate:
...
def resnet20(n_divs=1):
    return ResNet(BasicBlock, [3, 3, 3], n_divs=n_divs)


def resnet32(n_divs=1):
    return ResNet(BasicBlock, [5, 5, 5], n_divs=n_divs)


def resnet44(n_divs=1):
    return ResNet(BasicBlock, [7, 7, 7], n_divs=n_divs)


def resnet56(n_divs=1):
    return ResNet(BasicBlock, [9, 9, 9], n_divs=n_divs)


def resnet110(n_divs=1):
    return ResNet(BasicBlock, [18, 18, 18], n_divs=n_divs)


def resnet1202(n_divs=1):
    return ResNet(BasicBlock, [200, 200, 200], n_divs=n_divs)
...
class Encoder(ResNet):
    def __init__(self,  # block,
        ...
        super().__init__(block, num_blocks, in_channels, in_planes,  # num_classes,
                         ...
                         inplace_activation=inplace_activation, constant_dim=constant_dim, num_classes=0,
                         use_se=use_se)
