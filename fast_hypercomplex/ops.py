...
def get_modulus1(x, n_divs=4, dim=1):
    # represent hypercomplex as x = x0 + x' and get the modulus of x'
    # xs = torch.chunk(x, chunks=n_divs, dim=dim)
    left_params = [f'p{i + 1}' for i in range(x.ndim - 1)]
    right_params = [f'p{i + 1}' for i in range(x.ndim - 1)]
    left_params.insert(dim, '(n_divs c)')
...
def get_angles(x, n_divs=4, dim=1):
    # get the polar and extended polar angles of hypercomplex
    # xs = torch.chunk(x, chunks=n_divs, dim=dim)
    left_params = [f'p{i + 1}' for i in range(x.ndim - 1)]
    right_params = [f'p{i + 1}' for i in range(x.ndim - 1)]
    left_params.insert(dim, '(n_divs c)')
    ...
    x= rearrange(x, f"{' '.join(left_params)} -> {' '.join(right_params)}", n_divs=n_divs)
    # x = rearrange(x, 'b (n_divs c) h w -> n_divs b c h w', n_divs=n_divs)
    x_norm = (x ** 2).sum(0).sqrt_()
    x_norm1 = (x[1:] ** 2).sum(0).sqrt_()
    
    cos_theta = x[0]/(x_norm + 1e-3)
    cos_eta = x[1:]/(x_norm1 + 1e-3)
    return cos_theta.arccos_(), cos_eta.arccos_()
